...
Performs a variety of bulk actions, especially the ability to check whether
...
NETBOX = <YOUR NETBOX ADDRESS HERE>
    ...
    try:
        ...
        for element in names:
            try:
                if not element:
                    break
                print("renaming {}".format(element[0]))
...
def func_lookup(type='device', action='list'):
    """Calls fuction for device and action."""
    if action == 'query':
        func = action
    else:
        func = '_'.join((type, action))

    # will fail if doesn't exist or arguments not supplied
    try:
        return func_map[func]()
    except:
        print('Error: invalid argument combination: -t {}, -a {}'.format(
              arguments['type'], arguments['action']))
        sys.exit(1)
        return None
...
"""
    ...
    arguments['headers'] = True

    # =======================
    # query

    # list of all type_action combos for nbcli
    func_map = {
        'circuit_list': circuit_list,
        'device_delete': object_delete,
        'device_list': device_list,
        'device_locate': device_locate,
        'device_rename': change_name,
        'ip_list': ip_list,
        'prefix_list': prefix_list,
        'rack_list': rack_list,
        'serial_list': serial_list,
        'serial_locate': serial_locate,
        'vlan_list': vlan_list,
        }

    # 2018-02-11/DN: might not be needed
    valid_types = ['circuit', 'interface', 'ip', 'prefix', 'rack',
                   'serial', 'vlan']
    valid_actions = ['delete', 'export', 'list', 'locate', 'query',
                     'rename']

    # Begin type_action mapping
    if (arguments['query'] is not None):
        querysearch(arguments['query'])
    else:
        func_lookup(arguments['type'], arguments['action'])
